...
class ChangeRequestService extends cds.ApplicationService {
  async init() {
    ...
    const runRequestChecks = async (req, { propose, duplicates, scope = null, standard = false }) => {
      ...
      //
      // `renderRole` is the SCREEN's own role, separate from that: gating what a derivation may
      ...
      const [configured, properties, renderRole] = await Promise.all([
        configuredStages(),
        fieldPropertyStages(requesterContext(req)),
        resolveEffectiveRole(req, req.data.Role || null)
      ]);
